...
<template>
    <div class="container">
        ...
        <ul class="ul" v-show="city">
              <li v-for='n in search' :key="n.id" @click="tiaozhuan(n)">
                {{n.text}}
              </li>
        </ul>
        <div class="content">
            <div class="popular">
                ...
                <ul>
                  <li v-for="(item,index) in arr" :key="item.id" v-if="index < 15">
                    <button :class="{selected:selectCity == item.id}" @click="tiaozhuan(item)">{{item.district}}</button>
                    </li>
                </ul>
            </div>
            <div class="history">
              <h2>历史记录<button class="qingkong" @click="qingkong">清空</button></h2>
...
.popular h2,.history h2{
    font-weight: 400;
    font-size: 0.5rem;
}
.popular li button,.history li button{
  width: 1.3rem;
  height: 0.5rem;
  margin-right: 0.3rem;
